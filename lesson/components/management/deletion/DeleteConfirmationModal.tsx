        ...
        <View style={styles.shell}> 
          ...
          <LinearGradient colors={['#0d47a1','#1976d2']} start={{x:0,y:0}} end={{x:1,y:1}} style={styles.headerBar}> 
            <View style={styles.headerLeft}> 
              ...
              <View style={styles.headerTextCol}> 
                <Text style={styles.headerTitle} numberOfLines={1}>מחיקת שיעור</Text>
            ...
            <TouchableOpacity style={styles.closeBtn} onPress={onClose} accessibilityLabel="סגור חלון מחיקת שיעור">
          ...
          <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}> 
            ...
            <View style={styles.sectionCard}> 
              <Text style={styles.sectionLabel}>שיעור</Text>
              <Text style={styles.lessonTitle} numberOfLines={2}>{lesson.title}</Text>
              {lesson.description ? (
                <Text style={styles.bodyText} numberOfLines={4}>{lesson.description}</Text>
              ) : (
                <Text style={styles.placeholderText}>לא סופק תיאור.</Text>
              )}
            </View>
            ...
            <View style={styles.metricRow}> 
              <View style={[styles.metricChip, styles.metricChipEmphasis]}> 
                ...
                <Text style={[styles.metricChipText, styles.metricChipTextEmphasis]}>{isFree ? 'חינם' : formatPrice(lesson.price)}</Text>
              </View>
              {lesson.duration ? (
                <View style={[styles.metricChip, styles.metricChipEmphasis]}> 
            ...
            {capacity > 0 && (
              <View style={styles.sectionCard}> 
                ...
                <Text style={styles.capacityHint}>{registered === 0 ? 'עדיין אין משתתפים' : `${registered} משתתפ${registered > 1 ? 'ים' : ''} רשומים`}</Text>
              </View>
            )}
            ...
            {locationText ? (
              <View style={styles.sectionCard}> 
                ...
                <View style={styles.inlineRow}> 
                  <Entypo name="location-pin" size={18} color="#0d47a1" style={{ marginLeft:6 }} />
            ...
            <View style={[styles.sectionCard, styles.warningCard]}> 
              <View style={styles.warningHeaderRow}> 
                ...
                <Text style={styles.warningTitle}>מחיקה לצמיתות</Text>
              </View>
              <Text style={styles.warningBody}>שיעור זה וכל הרישומים המשויכים יוסרו לצמיתות. המשתתפים לא יראו אותו יותר. פעולה זו אינה ניתנת לביטול.</Text>
            </View>
          ...
          <View style={styles.footerBar}> 
            <TouchableOpacity style={styles.cancelBtn} onPress={onClose} disabled={isDeleting} accessibilityLabel="ביטול מחיקה"> 
              <Text style={[styles.cancelBtnText, isDeleting && { opacity:0.5 }]}>ביטול</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.dangerBtn, isDeleting && styles.dangerBtnDisabled]} onPress={handleDelete} disabled={isDeleting} accessibilityLabel="אישור מחיקת שיעור"> 
              {isDeleting ? (
...
const styles = StyleSheet.create({
  ...
  lessonTitle:{ fontSize:17, fontWeight:'800', color:'#0d47a1', marginBottom:10, letterSpacing:0.4, textAlign:'left', writingDirection:'rtl' },
