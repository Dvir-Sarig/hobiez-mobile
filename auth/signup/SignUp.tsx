...
export default function SignUpScreen() {
  ...
  const validateInputs = () => {
    ...
    if (!formData.name.trim()) { newErrors.name = 'שם הוא שדה חובה.'; isValid = false; }
    if (!formData.email || !/\S+@\S+\.\S+/.test(formData.email)) { newErrors.email = 'הכנס אימייל תקין.'; isValid = false; }
    if (!formData.password || formData.password.length < 6) { newErrors.password = 'מינימום 6 תווים.'; isValid = false; }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrors((prev) => ({ ...prev, general: error.message || 'שגיאה לא צפויה' }));
  ...
  return (
    <AuthLayout>
      <View style={styles.header}> 
        ...
        <Text style={styles.title}>הרשמה כ{role === 'client' ? 'לקוח' : 'מאמן'}</Text>
        <Text style={styles.subtitle}>
          {role === 'client' ? 'גלה והזמן שיעורים עם מאמנים מומחים.' : 'שתף מומחיות ובנה את המותג שלך.'}
        </Text>
      </View>
      <View style={styles.formCard}>
        ...
        <View style={[styles.inputGroup, errors.password && styles.inputGroupError]}> 
          ...
          <TextInput
            ...
            returnKeyType="done"
          />
          <TouchableOpacity onPress={() => setSecure(!secure)} style={styles.toggleSecure} accessibilityLabel={secure ? 'הצג סיסמה' : 'הסתר סיסמה'}>
        ...
        <TouchableOpacity style={[styles.primaryButton, isLoading && styles.buttonDisabled]} onPress={handleSubmit} disabled={isLoading} accessibilityLabel="צור חשבון Hobinet">
          {isLoading ? <ActivityIndicator color={tokens.colors.primaryDark} /> : <Text style={styles.primaryButtonText}>צור חשבון</Text>}
        </TouchableOpacity>

        <TouchableOpacity style={styles.secondaryGhostBtn} onPress={() => navigation.navigate('SignIn')} accessibilityLabel="מעבר להתחברות">
          <Text style={styles.secondaryGhostBtnText}>כבר יש לך חשבון? התחבר</Text>
        </TouchableOpacity>
      </View>
    </AuthLayout>
  );
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 30, color: tokens.colors.textOnDark, fontWeight: tokens.fontWeight.heavy as any, textAlign: 'left', writingDirection: 'rtl', letterSpacing: 0.6, marginBottom: tokens.space.sm },
  subtitle: { fontSize: 15, lineHeight: 22, color: tokens.colors.textSubtle, textAlign: 'left', writingDirection: 'rtl', maxWidth: 300 },
  formCard: { ...surfaces.glassCard, borderRadius: tokens.radius.xl, padding: tokens.space.xl, ...utils.shadowSoft },
  sectionLabel: { fontSize: 12, fontWeight: tokens.fontWeight.bold as any, letterSpacing: 0.8, color: tokens.colors.textOnDark, marginBottom: tokens.space.md, textAlign: 'left', writingDirection: 'rtl' },
  ...
  errorText: { color: tokens.colors.error, fontSize: 12, marginBottom: tokens.space.xs, fontWeight: tokens.fontWeight.medium as any, textAlign: 'left', writingDirection: 'rtl' },
  ...
  primaryButtonText: { color: tokens.colors.primary, fontSize: 16, fontWeight: tokens.fontWeight.bold as any, textAlign: 'left', writingDirection: 'rtl' },
  secondaryGhostBtn: { marginTop: tokens.space.md, paddingVertical: tokens.space.sm, alignItems: 'center' },
  secondaryGhostBtnText: { color: tokens.colors.textOnDark, fontSize: 14, fontWeight: tokens.fontWeight.semibold as any, textDecorationLine: 'underline', textAlign: 'left', writingDirection: 'rtl' },
});
