...
import { getLessonIcon, getLessonTypeDisplayName } from '../../../lesson/types/LessonType';
...
const UnregisterLessonModal: React.FC<Props> = ({
  ...
  const dateStr = new Date(lesson.time).toLocaleDateString('he-IL', { day: 'numeric', month: 'long', year: 'numeric' });
        ...
        <View style={styles.overlay}>
          <View style={styles.shell}>
              ...
              <View style={styles.headerLeft}> 
                ...
                <View style={styles.headerTextCol}> 
                  ...
                  <View style={styles.headerMetaRow}> 
                    ...
                    <Icon name="schedule" size={14} color="#bbdefb" style={{ marginStart:10 }} />
                    <Text style={styles.headerMetaText}>{timeStr}</Text>
                  </View>
                </View>
              </View>
              <TouchableOpacity style={styles.closeBtn} onPress={onClose} accessibilityLabel="סגור פרטי שיעור">
            ...
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.metricRow}> 
                ...
                {lesson.duration ? (
                  <View style={[styles.metricChip, styles.metricChipEmphasis]}> 
                    ...
                    <Text style={[styles.metricChipText, styles.metricChipTextEmphasis]}>{lesson.duration} דק׳</Text>
                  </View>
                ) : null}
              </View>
              ...
              {locationText ? (
                <View style={styles.sectionCard}> 
                  ...
                  <View style={styles.inlineRow}> 
                    <Entypo name="location-pin" size={18} color="#0d47a1" style={{ marginStart:6 }} />
              ...
              <View style={styles.sectionCard}> 
                ...
                <TouchableOpacity onPress={handleCoachPress} style={styles.coachRow} accessibilityLabel="צפה בפרופיל המאמן"> 
                  {renderCoachAvatar()}
                  <Text style={styles.coachNameText} numberOfLines={1}>{coach?.name || 'מאמן'}</Text>
                  <Icon name="chevron-left" size={20} color="#0d47a1" />
                </TouchableOpacity>
              </View>
            ...
            <View style={styles.footerBar}> 
              <TouchableOpacity style={styles.secondaryBtn} onPress={onClose} disabled={isProcessing} accessibilityLabel="שמור רישום"> 
                <Text style={[styles.secondaryBtnText, isProcessing && { opacity:0.5 }]}>שמור מקום</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.dangerBtn, isProcessing && styles.dangerBtnDisabled]} onPress={handleUnregisterPress} disabled={isProcessing} accessibilityLabel="התחל ביטול רישום"> 
                {isProcessing ? <ActivityIndicator size="small" color="#ffffff" /> : <Text style={styles.dangerBtnText}>ביטול רישום</Text>}
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  headerMetaText:{ fontSize:12.5, fontWeight:'700', color:'#ffffff', marginStart:4, letterSpacing:0.5, textAlign:'left', writingDirection:'rtl' },
  ...
  metricChipText:{ fontSize:12, fontWeight:'700', color:'#0d47a1', letterSpacing:0.3, textAlign:'left', writingDirection:'rtl' },
  ...
  capacityPillText:{ fontSize:13, fontWeight:'800', letterSpacing:0.5, textAlign:'left', writingDirection:'rtl' },
  capacityHint:{ fontSize:11, fontWeight:'600', color:'rgba(13,71,161,0.70)', letterSpacing:0.4, textAlign:'left', writingDirection:'rtl' },
  ...
  noticeTitle:{ fontSize:15, fontWeight:'800', color:'#0d47a1', letterSpacing:0.5, textAlign:'left', writingDirection:'rtl' },
  noticeBody:{ fontSize:13, fontWeight:'600', color:'#1e3a8a', lineHeight:19, textAlign:'left', writingDirection:'rtl' },
  ...
  secondaryBtnText:{ color:'#0d47a1', fontSize:14, fontWeight:'700', letterSpacing:0.4, textAlign:'left', writingDirection:'rtl' },
